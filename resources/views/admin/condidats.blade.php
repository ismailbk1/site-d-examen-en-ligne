...
<body>
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            <thead class="bg-200 text-900">
                                                <tr>
                                                    ...
                                                    <th class="sort" data-sort="age">Reponse vrais</th>
                                                    <th class="sort" data-sort="age">Reponse fais</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                            <tbody class="list">
                                                ...
                                                @foreach ($user as $index => $u)
                                                    <tr>
                                                        ...
                                                        <td class="email">{{ $u->name }}</td>
                                                        <td class="age">{{ $u->email }}</td>
                                                        @if (isset($arrayidf[$index]))
                                                        <td class="age">{{ $arraytotalv[$index ] }}</td>
                                                        <td class="age">{{ $arraytotalf[$index ] }}</td>
                                                      ...
                                                      @endif
                                                        <td>
                                                            <a type="button" data-bs-toggle="modal"
                                                                data-bs-target="#categomodif{{ $u->id }}"
                                                                class="btn btn-outline-success">Modifier</a>
                                                            <a type="button"
                                                                onclick="return confirm('Are you sure you want to delete?')"
                                                                class="btn btn-outline-danger"
                                                                href="/admin/question/destory/{{ $u->id }}">Supprimer</a>
                                                                <a type="button"
                                                                onclick="return confirm('Are you sure you want to delete?')"
                                                                class="btn btn-outline-danger"
                                                                href="/admin/question/destoryreponse/{{ $u->id }}">effacer reponse</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
  ...
  fin modal --}}
    ...
    @foreach ($user as $index => $u)
        <div class="modal fade" id="categomodif{{ $u->id }}" tabindex="-1"
            aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Modifier email : <span
                                class="text-primary">{{ $u->qst }}</span></h5><button class="btn p-1"
                            type="button" data-bs-dismiss="modal" aria-label="Close"><span
                                class="fas fa-times fs--1"></span></button>
                ...
                <form class="row g-3 "  action="/admin/question/update" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3"><label class="form-label" for="exampleFormControlInput1">question :
                            ...
                            <input name="question" value="{{ $u->qst }}" class="form-control" id="exampleFormControlInput1" type="text"
                               >
                        </div>
                        @error('questtion')
                            <div class="alert alert-danger"> {{ $message }}</div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input name="a" value="{{$u->a}}" type="text" class="form-control" id="validationCustom01" required>
                  
                      </div>
                      <div class="col-md-4">
                        <label for="validationCustom02" class="form-label">Choix B :</label>
                        <input name="b" value="{{$u->b}}" type="text" class="form-control" id="validationCustom02" required>
                     
                      </div>
                      <div class="col-md-4">
                        <label style="font-weight: 700" for="validationCustom01" class="form-label">C</label>
                        <input value="{{$u->c}}" name="c" type="text" class="form-control" id="validationCustom01" required>
                  
                      </div>
                      <div class="col-md-4">
                        <label for="validationCustom02" class="form-label">Choix D :</label>
                        <input value="{{$u->d}}" name="d" type="text" class="form-control" id="validationCustom02" required>
                     <input  type="hidden" name="id" value="{{$u->id}}">
                      </div>
                      <label for="validationCustom02" class="form-label">Reponse :</label>
                      <div>  <select class="form-select" name="ans"  aria-label="Default select example">
                        <option selected>{{$u->ans}}</option>
