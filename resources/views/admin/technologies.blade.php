...
<body>
                            ...
                            <div class="row align-items-center g-4">
                                ...
                                <div>
                                    <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal"
                                        data-bs-target="#exampleModal">Ajouter un Technologie</button>
                                </div>
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            <thead class="bg-200 text-900">
                                                <tr>
                                                    ...
                                                    <th class="sort" data-sort="age">Temp</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                            <tbody class="list">

                                                @foreach ($techs as $index => $t)
                                                    <tr>
                                                        <td class="name">{{ $index + 1 }}</td>
                                                        <td class="email">{{ $t->name }}</td>
                                                        <td class="age">{{ $t->description }}</td>
                                                        <td class="age">{{ $t->temp }}</td>
                                                        <td>
                                                            <a style="    width: 200px "  type="button" data-bs-toggle="modal"
                                                                data-bs-target="#categomodif{{ $t->id }}"
                                                                class="btn btn-outline-success">Modifier</a>
                                                            <a style="    width: 200px" type="button"
                                                                onclick="return confirm('Are you sure you want to delete?')"
                                                                class="btn btn-outline-danger"
                                                                href="/admin/technologie/destory/{{ $t->id }}">Supprimer</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Ajouter un Technologie </h5><button class="btn p-1"
                        type="button" data-bs-dismiss="modal" aria-label="Close"><span
                            class="fas fa-times fs--1"></span></button>

                </div>
      <form class="row g-3 "  action="/admin/technologie/store" method="POST">
                    @csrf  
                    <div class="modal-body">

                        <div class="mb-3"><label class="form-label" for="exampleFormControlInput1">Technologie
                            </label>
                            <input name="name" class="form-control" id="exampleFormControlInput1" type="text"
                                placeholder="tapper le nom de technologie...">
                        </div>
                        @error('questtion')
                      ...
                      <div style="display: block" >
                        <label style="margin-left: -3px ;color:black" for="validationCustom02" class="form-label">Temp :</label><br>
                        <input style="margin-left: 5px" name="temp" type="text" required>
    ...
    @foreach ($techs as $index => $t)
        <div class="modal fade" id="categomodif{{ $t->id }}" tabindex="-1"
            aria-labelledby="exampleModalLabel" aria-hidden="true"   >
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header" >
                        <h5 class="modal-title" id="exampleModalLabel">Modifier technologie : <span
                                class="text-primary">{{ $t->name }}</span></h5><button class="btn p-1"
                            type="button" data-bs-dismiss="modal" aria-label="Close"><span
                                class="fas fa-times fs--1"></span></button>

                    </div>
                    
                <form class="row g-3 "  action="/admin/technologie/update" method="POST" >
                    @csrf  
                        ...
                        <div class="mb-3" >
                            ...
                            border-radius: 5px;" name="name" value="{{ $t->name }}"  type="text"
                               >
                        </div>
                      ...
                      <div class="mb-3">
                        ...
                        margin-left: -5px;"  for="exampleFormControlInput1" class="form-label">Temp :</label>
                        <input style="    margin-left: 10px;
                        width: 200px;
                        border: 1px solid;" value="{{$t->temp}}" name="temp" type="text" class="form-control" id="exampleFormControlInput1" >
                      </div>
                      <div class="mb-3">
                        <label style="color: #522157;
                        margin-left: -5px;" for="exampleFormControlTextarea1" class="form-label">Description :</label>
                        <textarea name="description" class="form-control" id="exampleFormControlTextarea1" rows="3">{{$t->description}}</textarea>
                      </div>
